...
public class FileUtils {
     ...
     * @param extensions - an array of extensions, ex. {"java","xml"}. If this parameter is null,
     ...
     * which match an array of extensions. This method is based on listFiles(File, String[],
     * boolean).
     *
     * @param directory - the directory to search in
     * @param extensions - an array of extensions, ex. {"java","xml"}. If this parameter is null,
     ...
     * FileUtils.copyDirectory(srcDir, destDir, DirectoryFileFilter.DIRECTORY, false); Example: Copy
     * directories and txt files // Create a filter for ".txt" files IOFileFilter txtSuffixFilter =
     ...
     *
     * // Create a filter for either directories or ".txt" files FileFilter filter =
     * FileFilterUtils.orFileFilter(DirectoryFileFilter.DIRECTORY, txtFiles);
     *
     * // Copy using the filter FileUtils.copyDirectory(srcDir, destDir, filter, false);
     */
     ...
     *
     * Example: Copy directories only // only copy the directory structure
     * FileUtils.copyDirectory(srcDir, destDir, DirectoryFileFilter.DIRECTORY); Example: Copy
     * directories and txt files // Create a filter for ".txt" files IOFileFilter txtSuffixFilter =
     * FileFilterUtils.suffixFileFilter(".txt"); IOFileFilter txtFiles =
     * FileFilterUtils.andFileFilter(FileFileFilter.FILE, txtSuffixFilter);
     *
    ...
    /**
     * Compares the contents of two files to determine if they are equal or not. This method checks
     * to see if the two files are different lengths or if they point to the same file, before
     * resorting to byte-by-byte comparison of the contents. Code origin: Avalon
     *
     ...
     * checked using one Checksum instance if desired simply by reusing the same checksum object.
     * For example:
     *
     ...
     * will be created if they don't already exist. destination will be overwritten if it already
     * exists.
     */
    ...
    /**
     * Deletes a file. If file is a directory, delete it and all sub-directories. The difference
     * between File.delete() and this method are:
     *
     * •A directory to be deleted does not have to be empty. •You get exceptions when a file or
     * directory cannot be deleted. (java.io.File methods returns a boolean)
     *
     ...
     * sub-directories.
     *
     * The difference between File.delete() and this method are: •A directory to be deleted does not
     * have to be empty. •No exceptions are thrown when a file or directory cannot be deleted.
     *
     ...
     * @param createDestDir - If true create the destination directory, otherwise if false throw an
     * IOException
     */
    ...
    /**
     * Opens a FileInputStream for the specified file, providing better error messages than simply
     * calling new FileInputStream(file). At the end of the method either the stream will be
     * successfully opened, or an exception will have been thrown.
     *
    ...
    /**
     * Opens a FileOutputStream for the specified file, checking and creating the parent directory
     * if it does not exist. At the end of the method either the stream will be successfully opened,
     * or an exception will have been thrown. The parent directory will be created if it does not
     * exist. The file will be created if it does not exist. An exception is thrown if the file
     * object exists but is a directory. An exception is thrown if the file exists but cannot be
     * written to. An exception is thrown if the parent directory cannot be created.
     *
     ...
     */
    public static File getRealFileName(String baseDir, String absFileName) throws Exception {
        File ret = null;
     ...
     */
    static public void copyFile(String srcFile, String targetFile) throws Exception {
    ...
    public static void write(String pathName, String content, boolean append, String charset) throws IOException {
    ...
    public static boolean makeDirectory(File file) {
        /*
         * File parent = file.getParentFile(); if (parent != null) { boolean r= parent.mkdirs();
    ...
    /**
     * 清空指定目录中的文件。 这个方法将尽可能删除所有的文件，但是只要有一个文件没有被删除都会返回false。 另外这个方法不会迭代删除，即不会删除具有内容的子目录。
     *
     ...
     * 已经关闭流的连接
     */
    public static String readTxt(InputStream in, String charset) throws Exception {
     ...
     * @return 返回文本文件的内容
     */
    public static String readTxt(String filePathAndName, String charset) throws IOException {
     ...
     * 独占锁方式写入
     */
    public static void safeWrite(String pathName, String content, boolean append, String charset) throws IOException {
        ...
        try {
            ...
            while (true) {
                try {
                    fileLock = channel.tryLock();// true表示是共享锁，false则是独享锁定
                    if (fileLock != null) {
                        raf.skipBytes((int) raf.length());
                        raf.write(IOUtils.toByteArray(new StringReader(content), charset, false));
                        break;
                    } else {
                        try {
                            Thread.sleep(1000);
